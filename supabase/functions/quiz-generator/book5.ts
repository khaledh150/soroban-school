...
function randomBook5Chapter4(numQuestions = 10): Question[] {
  ...
  // Generate valid pairs where A (1-9) × B (1-9) = C (10-99)
  const validPairs: { A: number; B: number; C: number }[] = [];
  for (let A = 1; A <= 9; A++) {
    for (let B = 1; B <= 9; B++) {
      const C = A * B;
      if (C >= 10 && C <= 99) {
        validPairs.push({ A, B, C });
      }
    }
  }
  ...
  while (questions.length < numQuestions && attempts < 4000) {
    ...
    const qStr = `_ × ${pair.B} = ${pair.C}`;

    if (!uniqueSet.has(qStr)) {
      questions.push({ q: qStr, a: pair.A.toString() });
      uniqueSet.add(qStr);
    }
  }
  return fillQuestions(questions, numQuestions!);
